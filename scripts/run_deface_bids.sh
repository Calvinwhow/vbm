...
for T1_FILE in $T1_FILES; do
  ...
  NEW_DIR="$SUBJECT_DIR/deface"
  mkdir -p "$NEW_DIR"
  cp "$T1_FILE" "$NEW_DIR/"
  
  # 2) Run dicom to nii conversion
  if ls "$NEW_DIR"/*.nii 1> /dev/null 2>&1; then
    echo "Deface completed for: $T1_FILE. Skipping."
  else
    echo "Running CAT12 Deface on: $T1_FILE"
    cat_standalone.sh -b /root/scripts/cat_standalone_deface.m "$NEW_DIR/$T1_BASENAME"
  fi
  done
done
